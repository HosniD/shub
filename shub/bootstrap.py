...
def cli(project, target_dir):
    target_dir = os.path.join(os.getcwd(), target_dir or project)
...
def unzip_project(repo_zip, project, target_dir):
    ...
    project_filenames = [
        fn
        for fn in filenames
        if fn.startswith(repo_dirname + project['directory'])
    ]
    tempdir = tempfile.mkdtemp()
    repo_zip.extractall(path=tempdir, members=project_filenames)
    shutil.move(
        os.path.join(tempdir, repo_dirname, project['directory']),
        target_dir,
    )
